...
# The official Node image already ships matching headers under /usr/local. Point
# node-gyp there so self-hosted builders do not depend on nodejs.org connectivity
# (for example, a Docker host with an advertised but unreachable IPv6 route).
RUN --mount=type=cache,id=s/92ca8a61-c1ba-421f-a389-d48ac7258c2d-npm-cache,target=/root/.npm \
  npm ci --include=optional --no-audit --no-fund --legacy-peer-deps --ignore-scripts \
  && (cd node_modules/better-sqlite3 \
      && node /usr/local/lib/node_modules/npm/node_modules/node-gyp/bin/node-gyp.js rebuild --force_build=1 --nodedir=/usr/local) \
